...
describe Bodhi::Type do
  ...
  describe "#patch!(params)" do
    ...
    it "raises Bodhi::ApiErrors if the type could not be patched" do
      type = Bodhi::Type.new(bodhi_context: context, name: "test")
      expect{ type.patch!([{}]) }.to raise_error(Bodhi::ApiErrors)
    end

    it "updates the type with the given patch arguments" do
      type = Bodhi::Type.new(
        bodhi_context: context,
        name: "AutoTest_Type1",
        properties: { foo: { type: "String", required: true }, bar: { type: "String", required: true }},
      ...
      expect{ type.patch!([{op: "add", path: "/properties/foo", value: {type: "String"}}]) }.to_not raise_error
      type.delete!
    end
  end
  ...
  describe ".factory" do
    ...
    describe "#create" do
      ...
      it "saves a type to the cloud and returns Bodhi::Type" do
        type = Bodhi::Type.factory.create(bodhi_context: context, namespace: context.namespace, name: "AutoTest_Type1", properties: { foo: { type: "String", required: true }, bar: { type: "Integer", required: true, multi: true } } )
